...
using EShopOnAbp.CurrencyService.Web;
...
[DependsOn(
    typeof(CurrencyServiceWebModule),
...
public class CurrencyServiceWebHostModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                typeof(CurrencyServiceResource),
                typeof(CurrencyServiceDomainSharedModule).Assembly,
                typeof(CurrencyServiceApplicationContractsModule).Assembly,
                typeof(CurrencyServiceWebHostModule).Assembly
            );
        });
    }
    ...
    private void ConfigureMenu(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new CurrencyServiceWebHostMenuContributor(configuration));
        });
    }
    ...
    private void ConfigureAutoMapper()
    {
        ...
        {
            options.AddMaps<CurrencyServiceWebHostModule>();
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<CurrencyServiceWebModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}EShopOnAbp.CurrencyService.Web", Path.DirectorySeparatorChar)));
            });
        }
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "CurrencyService API");
        });
